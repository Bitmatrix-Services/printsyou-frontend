...
import {cartModalSchema} from '@components/globals/cart/add-to-cart-modal.component';
...
export const UpdateCartModal = () => {
  ...
  useEffect(() => {
    ...
    if (cartState.selectedProduct) {
      ...
      const priceTypeExists = cartState.selectedProduct.priceGrids.some((item: PriceGrids) => item.priceType);
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const res = await axios.get('/s3/signedUrl', {params: data});
                ...
                <div>
                  ...
                  <label className="w-fit flex group text-sm font-bold bg-secondary-500 hover:bg-primary-500 text-white cursor-pointer">
                    <span className="group-hover:bg-primary-600 bg-secondary-600 text-center px-5 py-5">
                      <Image height={15} width={15} src="/assets/icon-upload-white.png" alt="artwork upload" />
                    </span>
