...
const FeatureSection = () => {
  return (
    <section className="bg-white my-8">
      <Container>
        <div className="border border-[#ddd] lg:divide-x divide-[#ddd] rounded-lg p-4 flex lg:justify-between flex-wrap">
          {servicesData.map(item => (
            <div key={item.id} className="p-4 py-6">
              <div className="flex items-center gap-4">
                <Image width={36} height={36} src={item.iconSrc} alt=".." />
                <div>
                  <h6 className="font-semibold text-headingColor capitalize">
                    {item.title}
                  </h6>
                  <span className="font-normal text-sm text-mute">
                    {item.description}
                  </span>
                </div>
              </div>
